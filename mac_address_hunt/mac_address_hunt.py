...
maddr = str(input("What Mac-Address: e.g 1111.1111.1111 "))
...
def get_mac_address(host, username, password):
    ...
    mac_address = net_connect.send_command("show mac address-table address" + " " + maddr)

#    print("The Requested Mac-Address is learned on: " + "       " + (hostname))
...
with open('/automater/backup/backup_ios.csv') as csvfile:
    ...
    while rows >= 2:
        ...
        if ip_ping == None:
            ...
            downDeviceOutput = open("/automater/mac_address_hunt/" + fileName, "a")
            downDeviceOutput.write(str(ip) + " " + "is down! on " + str(today) + "\n")
